...
def write_str_data(start, end, days, el, op):
    print_str = ' %s %2s %5s START : %s END : %s %7s DAYS' % (el, op, str(opts.threshold), str(start), str(end), str(days))
    return print_str
    ...
    for i, date_val in enumerate(in_data):
        ...
        if gap and day_cnt !=0: # we are in middle of run and need to stop it
            ...
            if day_cnt >= opts.min_run:
                ...
                print_str1 = write_str_data(convert_date(run_start), convert_date(run_end), str(day_cnt), el, op)
        ...
        if flag == 0 and day_cnt == 0: #Beginning of a run
            ...
            if day_cnt >= opts.min_run:
                update_run_cnt(run_cnt, day_cnt)
                print_str = write_str_data(convert_date(run_start), convert_date(run_end), str(day_cnt), el, op)
                write_or_print(print_str, pr_flag)
            day_cnt = 0
            flag = 0
